...
사실 입출력은 우리가 짜는 명령어만으로는 구현할 수 없다. 덧셈, 뺄셈 등등으로는 파일에 내용을 작성하는 등의 동작이 불가능하기 때문이다. 이러한 동작은 시스템 콜(system call)로서 이루어진다. OS에서 제공하는 API를 호출하여 원하는 내용을 작성할 수 있도록 OS에게 알려주면, OS에서 출력을 실행하고, 실행 결과를 알려준다. 
